...
|---|---|---|---|
| `hashLength` | `2` | integer 2–4 | Characters per line hash in `read`/`grep` output and `edit` anchors. Longer hashes reduce the chance that a stale edit slips through undetected, at the cost of extra tokens on every line and every anchor. They do **not** reduce `[E_STALE_ANCHOR]` errors — stale anchors are rejected either way. |
...
`grep` uses ripgrep to search files and returns every matched line as a `LINE#HASH:content` anchor in the same format as `read` output. These anchors can be passed directly into `edit` without a prior `read`, closing the grep→edit flow without an extra round-trip.
